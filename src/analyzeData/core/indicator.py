...
class Indicator:
    ...
    EconDataset['지표명'] 으로 접근하여 사용.

    Examples
    --------
    >>> ind = ds['총지수']
    >>> ind.yoy()          # 전년동기 대비 변화율 Series
    ...
    def __repr__(self):
        return f"Indicator('{self.name}', {self._s.index.min().date()}~{self._s.index.max().date()}, n={len(self._s)})"
    ...
    def yoy(self, periods: int = 4) -> pd.Series:
        """전년 동기 대비 변화율 (%). 분기 데이터 기준 periods=4."""
    ...
    def cumulative_change(self, base_period: Optional[str] = None) -> pd.Series:
        """기준 시점 대비 누적 변화율. base_period 미지정 시 시작점 기준."""
