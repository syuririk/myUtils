...
class HeatmapChart(BaseChart):
    ...
    >>> fig = hm.yoy_heatmap()
    >>> fig = hm.rolling_corr_heatmap('총지수', '식료품')
    >>> fig.show()
    """
    ...
    def yoy_heatmap(
        ...
        fig.update_layout(
            **self._base_layout(title=title),
            height=max(300, len(yoy.columns) * 60),
    ...
    def rolling_corr_heatmap(
        ...
        fig.update_layout(
            **self._base_layout(
                ...
                xaxis_title="날짜", yaxis_title="Window",
            ),
            height=max(300, len(windows) * 60),
            xaxis=dict(tickangle=45),
        )
        return fig
